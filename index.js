...
app.get('/check', (req, res) => {
    ...
    // Lessen die nu bezig zijn (gestart maar nog niet voorbij)
    const bezig = lessenCache
        .filter(l => l.full_start <= nu && l.full_end > nu)
        .map(l => ({
            tijd: l.start_tijd,
    ...
    // Lessen die nog komen (binnen 24 uur)
    const komend = lessenCache
...
async function updateHomeyRotation() {
    ...
    const uur = nuDate.getHours() + nuDate.getMinutes() / 60;
    const isNacht = uur >= 23.25 || uur < 6.0;

    // 's Nachts alleen op het hele uur uitvoeren
    if (isNacht && nuDate.getMinutes() !== 0) return;
